...
if ($_SERVER['REQUEST_METHOD'] =='POST') {
    ...
    if (empty($firstname) || empty($lastname) || empty($username) || empty($email) || empty($phoneNo) || empty($password)) {
        ...
        $select = "SELECT username FROM applicants WHERE username='$username'";
        ...
        if (mysqli_num_rows($query) > 0) {
            ...
            $select = "SELECT phone FROM applicants WHERE phone='$phoneNo'";
            $query = mysqli_query($con, $select);
            if (mysqli_num_rows($query) > 0) {
                ...
                $select = "SELECT email FROM applicants WHERE email='$email'";
                $query = mysqli_query($con, $select);
                if (mysqli_num_rows($query) > 0) {
                    ...
                    $insert = "INSERT INTO applicants(first_name, last_name, username, phone, email,password)
